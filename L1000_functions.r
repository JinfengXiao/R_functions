...
# Inputs:
#   pert_c: Generated by cluster.r
#   ns: number of simulations
shared_edges_random_split = function(pert_c,ns=10000){
  stopifnot(max(as.integer(pert_c[,"cluster"]))==2);
  N = nrow(pert_c);
  n = sum(pert_c[,"cluster"]==1);
  pert_id_uniq = unique(pert_c[,"pert_id"]);
  shared_edges = rep(0,ns);
  total_edges = 0;
  for(i in 1:ns){
    # Randomly permute the assigned clusters
    pert_c[,"cluster"] = pert_c[sample(N),"cluster"];
    for(j in 1:length(pert_id_uniq)){
      exp_c = pert_c[pert_c[,"pert_id"]==pert_id_uniq[j],"cluster"];
      total_edges = total_edges + length(exp_c)*(length(exp_c)-1)/2;
      shared_edges[i] = shared_edges[i] + sum(exp_c==1)*sum(exp_c==2);
    }
    #print(i);
  }
  return(shared_edges);
